...
using namespace std;
short BCHCode::Array_Table[4096] = { 0 };								//读表的数据结构，下标为对应的伴随式（12-bit），值为相应的错误位置
char**  BCHCode::before = NULL;											//未注入错误之前与后来注入错误后完成修复工作，进行比较，看是否恢复正确

vector<vector<int> > BCHCode::adjointMatrix = {
...
vector<vector<int> > BCHCode::Generate_Matrix = {
	{ 0, 4, 5, 8, 9 },					//d0参与这么多校验
...
vector<vector<int> > BCHCode::Adjoint_Matrix = {
	{ 0, 4, 5, 8, 9 },		//d0参与的校验
	{ 1, 5, 6, 9, 10 },
	{ 2, 6, 7, 10, 11 },
	{ 0, 4, 5, 7, 10, 11 },
					...
					for (int k = 63; k >= 0; k--) {
						...
						if (IsOne(lres[i + j * (UnitSize / 8)], k)) {
							...
							if (result == 0) {
								...
								flag = 0;													//无错
								cout << "the buffer " << num <<" Using the inner decode success" << endl;
								...
								cout << "the buffer " << num <<" have many errors, the single code can't  repair,please choose other codes failure" << endl;
								flag = 1;
								break;
							}
						}
...
void BCHCode::testCode()
{
	...
	for (int i = 0; i < 1; i++) {
		...
		before = new char*[StripeNum];							//未注入错误之前与后来注入错误后完成修复工作，进行比较，看是否恢复正确
		for (int i = 0; i < StripeNum; i++) {					//进行初始化的一些操作
