...
void BCHCode_XOR::Decode()
{
	...
	for (int i = 0; i < Adjoint_Matrix.size(); i++) {
		for (int j = 0; j < Adjoint_Matrix[i].size(); j++) {
			xor_basic(res + Adjoint_Matrix[i][j] * UnitSize, buffer[0] + i * UnitSize, UnitSize);	//这样写对不对？
		}
	}
					...
					if (IsOne(lres[i + j * (UnitSize / 8)], k)) {
						...
						if (result == 0) {
							...
							for (int m = 0; m < CodeLen; m++) {
								if (IsOne(llbuffer[0][i + m * (UnitSize / 8)], k)) {
							...
							for (int m = 1; m < StripeNum + 1; m++) {
								int res2 = this->BCHDecode(testGroupInput[m], testGroupOutput[m]); 						//取到后，对本组的其他条纹进行解码
