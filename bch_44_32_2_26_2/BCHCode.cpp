...
const unsigned long number_strip = 50000000; //设置的数据量 50000000组大概是1GB的数据量
vector<bitset<dataLen> > testSecInput(number_strip * 4, bitset<dataLen> (0)); //输入 以四个编码条纹为一组 进行第二级的纠错
vector<bitset<codeLen> > testSecOutput(number_strip * 4, bitset<codeLen> (0)); //输出 以四个编码条纹为一组 进行第二级的纠错
vector<bitset<codeLen> > testSecDeInput(number_strip * 4, bitset<codeLen> (0)); //进行编码 输入 以四个编码条纹为一组 进行第二级的纠错
vector<bitset<codeLen> > testSecDeOutput(number_strip * 4, bitset<codeLen> (0)); //进行编码 输出 以四个编码条纹为一组 进行第二级的纠错
vector<bitset<codeLen> > testXOR(number_strip, bitset<codeLen> (0));
vector<bitset<codeLen> > testXORDeInput(number_strip, bitset<codeLen> (0));
vector<bitset<codeLen> > testXORDeOutput(number_strip, bitset<codeLen> (0));

unordered_map<bitset<parityLen>, bitset<codeLen>> errTable;
unsigned long flag[number_strip * 4] = {0};
...
void BCHCode::ErrorCorrection() {
    ...
    for(int i = 0; i < testSecDeOutput.size(); i++) {
        if(!flag[i]  && testSecDeOutput[i] == testSecOutput[i] ) {
            ...
            if(testXORDeOutput[Group] == testXOR[Group]) { //证明本组的XOR无错误
                ...
                //做异或校验
                for(int k = Group * 4; k < Group * 4 + 4; k++) {
                    if(k != i) {
...
//        }
    ...
    for(int i = 0; i < testSecInput.size(); i++) {
        Group = i / 4;
        for (int j = 0; j < blocksize; j ++) {
            if(testSecOutput[i][j]) {
                testXOR[Group].flip(j);
            }
        }
    }
    gettimeofday(&end_time, NULL);
