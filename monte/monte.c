...
#define THRESHOLD 36500									//阈值，1000年不出问题，就认为超可靠
...
#define SEU_MBU 10000										//多少次SEU会产生一次MBU, 1000表示1000次SEU会发生一次MBU
...
event event_heap[MEM_SIZE * 2];								//事件
int events;													//事件数目
...
int isFailed(int n, int method)
{
	...
	if(method == BCH_XOR) {																//两级冗余：如果有超过一个条纹出现了两个以上的错误，则无法修复
